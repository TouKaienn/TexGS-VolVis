...
class MovableGroup:
   ...
   def create_group(self):
      ...
      self.title_text = dpg.add_text(self.title, parent=self.group, indent=self.title_indent, tag=f"title_text_{self.group}")
      
      self.title_input = dpg.add_input_text(
         default_value=self.title, parent=self.group, show=False, indent=0,
         on_enter=True, callback=self.rename_title, tag=f"title_input_{self.group}"
      )

      with dpg.item_handler_registry(tag=f"title_click_handler_{self.group}"):
         dpg.add_item_clicked_handler(callback=self.enable_text_edit)
      dpg.bind_item_handler_registry(self.title_text, f"title_click_handler_{self.group}")

      return self.group

   def enable_text_edit(self, sender, app_data):
      dpg.configure_item(self.title_text, show=False)  
      dpg.configure_item(self.title_input, show=True)  
      dpg.focus_item(self.title_input)

   def rename_title(self, sender, app_data):
      new_title = app_data.strip()
      if new_title:
         self.title = new_title
         dpg.set_value(self.title_text, new_title)
      dpg.configure_item(self.title_input, show=False)  
      dpg.configure_item(self.title_text, show=True)
