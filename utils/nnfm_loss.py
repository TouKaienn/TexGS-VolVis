...
class NNFMLoss(torch.nn.Module):
    def __init__(self, device, n_cuts=16):
        super().__init__()
        ...
        torchvision.transforms.Resize(size=self.clip_model_input_size, max_size=None, antialias=None),
        torchvision.transforms.CenterCrop(size=(self.clip_model_input_size, self.clip_model_input_size)),
    ...
    def set_style_img(self, style_img, un_resized_style_img=None):
        self.style_img = style_img
        
        with torch.no_grad():
            # for image_prompt in style_img:
            image_embed = self.clip_model.encode_image(self.preprocess(un_resized_style_img))
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            s_feats_all = self.get_feats(self.style_img, all_layers)
        ...
        if "clip_loss" in loss_names:
            if self.n_cuts > 1:
                ...
                for _ in range(self.n_cuts):
                    ...
                    cutout = outputs[:, :, offsety : offsety + size, offsetx : offsetx + size]
                    cutouts.append(torch.nn.functional.adaptive_avg_pool2d(cutout, self.clip_model_input_size))
                outputs = torch.cat(cutouts)

            input_embed = self.clip_model.encode_image(self.preprocess(outputs))  
            ...
            loss_dict["clip_loss"] += dists.mean()
