...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, init_2DGS_path, partialEditing=False):
    ...
    ip2p = MultiViewsEditor(ip2p_device, ip2p_use_full_precision=args.ip2p_use_full_precision)
    ...
    edit_cams = None
    opacity_factor_schedule = args.opacity_factor_schedule

    progress_bar = tqdm(range(first_iter, opt.iterations), desc="Training progress")
    first_iter += 1
    for iteration in range(first_iter, opt.iterations + 1):        
        transform_dict['opacity_factors'][0].opacity_factor = opacity_factor_schedule[min(len(opacity_factor_schedule)-1, iteration//500)]
        
        iter_start.record()
        if ((iteration-1) % args.edit_steps) == 0:
            
            edit_cams = sample_edit_viewpoints(iteration, args, opt, scene, ip2p, pipe, background, transform_dict, gaussians, render_fn)
    ...
    if dataset.eval:
        ...
        os.makedirs(scene.model_path+f"/{args.edit_name}/test", exist_ok=True)
        os.makedirs(scene.model_path+f"/{args.edit_name}/train", exist_ok=True)
        gaussExtractor.reconstruction(scene.getTestCameras())
        gaussExtractor.export_image(scene.model_path+f"/{args.edit_name}/test")
        gaussExtractor.reconstruction(scene.getTrainCameras())
        gaussExtractor.export_image(scene.model_path+f"/{args.edit_name}/train")


def sample_edit_viewpoints(iteration, args, opt, scene, ip2p, pipe, background, transform_dict, gaussians, render_fn):
    UnedtiedTrainCameras = scene.getTrainCameras()
    total_view_num = len(INIT_VIEW_IDX)

    sorted_camera_idx = [INIT_VIEW_IDX[idx] for idx in sort_the_cameras_idx([UnedtiedTrainCameras[idx] for idx in INIT_VIEW_IDX])]
    sampled_camera = [UnedtiedTrainCameras[idx] for idx in sorted_camera_idx]
    
    
    t_max_step = args.add_noise_schedule
    ip2p.max_step = t_max_step[min(len(t_max_step)-1, iteration//args.edit_steps)]

    
    
    with torch.no_grad():
        batch_image_cond = [cam.original_image.unsqueeze(0) for cam in sampled_camera]
        masks = [cam.gt_alpha_mask.permute(2,0,1).unsqueeze(0) for cam in sampled_camera]
        cams = sampled_camera
        batch_images = []
        for idx, viewpoint_cam in enumerate(cams):
            render_pkg = render_fn(iteration, viewpoint_cam, gaussians, pipe, background,
                        opt=opt, is_training=True, transform_dict=transform_dict)
            batch_images.append(render_pkg["render"].permute(2,0,1).unsqueeze(0))
        
        batch_images = torch.cat(batch_images).to("cuda")
        batch_image_cond = torch.cat(batch_image_cond).to("cuda")
        batch_img_masks = torch.cat(masks).to("cuda")

        text_embedding = ip2p.pipe._encode_prompt(
        args.text_prompt, device="cuda", num_images_per_prompt=batch_images.shape[0], do_classifier_free_guidance=True, negative_prompt="strong light, Bright light, intense light, dazzling light, brilliant light, radiant light, Shade, darkness, silhouette, dimness, obscurity, shadow, glasses")
        batch_images = batch_images.permute(0, 2, 3, 1) # BHWC
        batch_image_cond = batch_image_cond.permute(0, 2, 3, 1)
        batch_img_masks = batch_img_masks.permute(0, 2, 3, 1)
        if iteration == 1:
            batch_images = deepcopy(batch_image_cond)
        edited_image = ip2p.edit_image(
            rgb=batch_images,
            cond_rgb=batch_image_cond,
            text_embeddings=text_embedding,
            cams=cams,
            guidance_scale=args.guidance_scale,
            image_guidance_scale=args.image_guidance_scale,
            img_mask=batch_img_masks
        )
        
        for idx, edited_image in enumerate(edited_image):
            # resize to original image size 
            edited_image = edited_image.to(batch_images[idx].dtype)
            sampled_camera[idx].original_image = edited_image.squeeze()
            save_img(edited_image, f"./log/cam_{idx}.png")
    return sampled_camera
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_steps', type=int, default=3000, help="how many GS steps between dataset updates")
    parser.add_argument('--edit_batch_size', type=int, default=5, help="how many cams in one edit batch")
    parser.add_argument('--max_num_cams', type=int, default=12, help="how many cams in one edit epoch")
    ...
    parser.add_argument("--add_noise_schedule", nargs="+", type=int, default=[999, 999, 999])
    parser.add_argument('--opacity_factor_schedule', nargs="+", type=int, default=[1.5, 1.0, 0.75, 0.5, 0.25]) #[2.0, 1.5, 1.0, 0.75, 0.5]
