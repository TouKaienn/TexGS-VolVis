...
class LearningPaletteColor:
    ...
    def set_black(self):
        with torch.no_grad():
            self.palette_color.data = torch.zeros(3).float().cuda()
    ...
    def create_from_ckpt(self, checkpoint_path, restore_optimizer=False):
        (model_args, first_iter) = torch.load(checkpoint_path, weights_only=True)
        with torch.no_grad():
            self.palette_color = torch.clamp(model_args[0], min=0.0, max=1.0)

        return first_iter

    def set_palette_color(self, palette_color):
        with torch.no_grad():
            self.palette_color.data = palette_color
    ...
    def load_palette_color(self, img_dir_path):
        def extract_palette(img_dir):
            ...
            for img_file in tqdm(img_files,leave=False):
                ...
                mask = img[:, :, 3] if img.shape[2] == 4 else 255
                colors = img[:, :, :3]  
