...
def texGS_render_woLight(iteration, viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, \
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, is_training=False, transform_dict=None):
    ...
    texture_channels = 4 # R,G,B, RGB^2 (for color sparsity loss)
    texture = torch.zeros((total_size, texture_channels), device=means3D.device)
    teture_dc = texture_dc.get_texture()
    ...
    if (len(texture_num_GSs_TFs) > 1) and (len(palette_color_transforms) > 1): # for multi GS rendering
        ...
        for idx, GS_nums in enumerate(texture_num_GSs_TFs):
            ...
            end_texture_idx = texture_dims[accumulate_GS_nums-1,2] + start_texture_idx
            texture[start_texture_idx:end_texture_idx,0:3] = teture_dc[start_texture_idx:end_texture_idx,:] + palette_color_transforms[idx].palette_color.clamp(0,1).detach()
            start_texture_idx = end_texture_idx
    else: # for individual GS optimization
        texture[:,0:3] = teture_dc + palette_color_transforms[0].palette_color.clamp(0,1).detach()
    
    texture[:,3] = torch.norm(teture_dc, dim=-1)
    ...
    all_outputs = custom_texture_gaussians(gaussian_rgbs, texture, opacity, (1<<9) | (1<<10))
    out_img, out_depth, out_reg, out_alpha, out_texture, out_normal = all_outputs[:6]

    rgb = out_img + out_texture[:,:,0:3] + (1 - out_alpha[:,:,None]) * bg_color[None,None,:]
    rgb = torch.clamp(rgb, min=0.0, max=1.0)
    offset_color_norm_render = out_texture[:,:,3]
    ...
    if not is_training:
        features = torch.cat([diffuse_factor, shininess, ambient_factor, specular_factor], dim=-1)
        reg_outputs = custom_texture_gaussians(features, texture, opacity, (1<<9) | (1<<10) | (1<<15))
        out_relight_reg, out_normal = reg_outputs[0], reg_outputs[5]
        render_diffuse_factor, render_shininess, render_ambient_factor, render_specular_factor = out_relight_reg.split([1,1,1,1], dim=-1)
        ...
        rets.update({
                ...
                "diffuse_factor": render_diffuse_factor,
                "shininess": render_shininess,
                "ambient_factor": render_ambient_factor,
                "specular_factor": render_specular_factor,
        })
