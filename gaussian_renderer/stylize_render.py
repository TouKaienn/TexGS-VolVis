...
def calculate_loss(iteration, viewpoint_camera, pc, results, opt, text_editing, img_editing, nnfm_loss_fn):
    """
    ...
    if ((img_editing) and (nnfm_loss_fn is not None)):
        nnfm_loss = nnfm_loss_fn(outputs=render_img.unsqueeze(0), contents=gt_image.unsqueeze(0), loss_names=["nnfm_loss", 'clip_loss'])
        loss += nnfm_loss["nnfm_loss"]*local_weight
        loss += nnfm_loss["clip_loss"]*(1-local_weight)
        tb_dict['nnfm_loss'] = nnfm_loss["nnfm_loss"]*local_weight
    return loss, tb_dict

def stylize_render(iteration, viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, \
    override_color = None, opt: OptimizationParams = False, is_training=False, transform_dict=None, text_editing=True, img_editing=False, nnfm_loss_fn=None):
    
    results = render(viewpoint_camera, pc, pipe, bg_color, scaling_modifier, override_color, transform_dict=transform_dict, is_training=is_training, img_editing=img_editing)
    if is_training:
        loss, tb_dict = calculate_loss(iteration, viewpoint_camera, pc, results, opt, text_editing, img_editing, nnfm_loss_fn)
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, is_training=False, transform_dict=None, img_editing=False):
    """
    Render the scene. 
    ...
    texture[:,0:3] = texture_values

    texture_info = (num_points, 1, texture_channels)

    
    colors, extra_results, opacity = rendering_equation_BlinnPhong_python(
        palette_color_transforms, opacity_transforms, opacity, diffuse_factor, shininess, ambient_factor, specular_factor, normal, viewdirs, incidents_dirs, num_GSs_TF)
    ...
    def custom_texture_gaussians(custom_rgbs, custom_texture, custom_opacities, texture_infomation, custom_settings):
        custom_outputs = texture_gaussians(
            ...
            specular_intensity, #multiply 0.5 for text editing
            means3D,
            scales,
            1,
            quats,
    ...
    if not img_editing:
        all_outputs = custom_texture_gaussians(gaussian_rgbs, texture, opacity, texture_info, (1<<9) | (1<<10)| (1<<15))
        ...
        all_outputs = custom_texture_gaussians(gaussian_rgbs, texture, opacity, texture_info, (1<<9) | (1<<10)| (1<<15) | (1<<27))
        out_img, out_depth, out_reg, out_alpha, out_texture, out_normal = all_outputs[:6]
        rgb = out_texture[:,:,0:3] + (1 - out_alpha[:,:,None]) * bg_color[None,None,:]
        rgb = torch.clamp(rgb, min=0.0, max=1.0)


    out_depth = out_depth*out_alpha
...
def rendering_equation_BlinnPhong_python(palette_color_transforms, opacity_transforms, opacity, diffuse_factor, shininess, ambient_factor, specular_factor, normals, viewdirs,
    ...
    if len(num_GSs_TFs) > 0: # for multi GS rendering
        start_GS = 0
    ...
    "ambient_diffuse_intesnity": ambient_factor+diffuse_intensity,
