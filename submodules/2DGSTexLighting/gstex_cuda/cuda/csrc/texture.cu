...
__global__ void texture_forward(
    ...
    const float* __restrict__ ambient_diffuse_intensity, //* {N, 1}
    const float* __restrict__ specular_intensity, //* {N, 1}
    ...
    //* Added new mode, use relighting or not based on 27th digit
    const bool phong_mode = (settings & (1<<27));
    ...
    __shared__ float ambient_diffuse_batch[MAX_BLOCK_SIZE];
    __shared__ float specular_batch[MAX_BLOCK_SIZE];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx < range.y) {
            ...
            ambient_diffuse_batch[tr] = ambient_diffuse_intensity[g_id];
            specular_batch[tr] = specular_intensity[g_id];
        }
        ...
        for (int i = 0; (i < batch_size); ++i) {
            ...
            if (!done) {
                ...
                const float ambient_diffuse_i = ambient_diffuse_batch[i];
                const float specular_i = specular_batch[i];
                ...
                if (!skip && !done) {
                    ...
                    for(int k = 0; k < texture_channels; k++) {
                        ...
                        if (phong_mode) {
                            val = std::max(val * ambient_diffuse_i + specular_i, 0.0f);
                        }
...
__global__ void texture_backward(
    ...
    __shared__ float opacities_batch[MAX_BLOCK_SIZE];
    __shared__ float ambient_diffuse_batch[MAX_BLOCK_SIZE];
    __shared__ float specular_intensity_batch[MAX_BLOCK_SIZE];
    __shared__ float3 scales_batch[MAX_BLOCK_SIZE];
    __shared__ float4 quats_batch[MAX_BLOCK_SIZE];
    __shared__ float3 rgbs_batch[MAX_BLOCK_SIZE];
    __shared__ int3 dims_batch[MAX_BLOCK_SIZE];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            vmap_batch[tr] = vmaps[g_id];
            ambient_diffuse_batch[tr] = ambient_diffuse_intensity[g_id];
            specular_intensity_batch[tr] = specular_intensity[g_id];
        }
        // wait for other threads to collect the gaussians in batch
        block.sync();
        // process gaussians in the current batch for this pixel
        ...
        for (int i = max(0,batch_end - warp_bin_final); i < batch_size; ++i) {
            ...
            if(valid){
                ...
                const float ambient_diffuse_i = ambient_diffuse_batch[i];
                const float specular_i = specular_intensity_batch[g_id];
                ...
                float v_alpha = 0.f;
                {
                    ...
                    for(int k = 0; k < texture_channels; k++) {
                        ...
                        // if (phong_mode) {
                            // if (val * ambient_diffuse_i + specular_i < 0.0f) {
                                // v_val = 0.f;
                            // } else {
                                // v_val = v_val * ambient_diffuse_i;
                            // }
...
texture_forward_tensor(
    ...
    const torch::Tensor &ambient_diffuse_intensity, //* for phong shading
    const torch::Tensor &specular_intensity, //* for phong shading
    ...
    CHECK_INPUT(ambient_diffuse_intensity);
    CHECK_INPUT(specular_intensity);
    CHECK_INPUT(means);
    CHECK_INPUT(scales);
    CHECK_INPUT(quats);
    CHECK_INPUT(uv0);
    ...
    texture_forward<<<tile_bounds_dim3, block_dim3>>>(
        ...
        ambient_diffuse_intensity.contiguous().data_ptr<float>(),
        specular_intensity.contiguous().data_ptr<float>(),
...
std::
    ...
    texture_backward_tensor(
        ...
        const torch::Tensor &colors,
        const torch::Tensor &opacities,
        const torch::Tensor &ambient_diffuse_intensity,
        const torch::Tensor &specular_intensity,
        const torch::Tensor &means,
        const torch::Tensor &scales,
        const float glob_scale,
        const torch::Tensor &quats,
