...
def texture_gaussians(
    ...
    ambient_diffuse_intensity: Float[Tensor, "*batch 1"],
    specular_intensity: Float[Tensor, "*batch 1"],
    ...
    Args:
        ...
        ambient_diffuse_intensity (Tensor): ambient diffuse intensity for phong shading (Note: Only used for forward, no grad)
        specular_intensity (Tensor): specular intensity for phong shading (Note: Only used for forward, no grad)
    ...
    return func(
        ...
        colors.contiguous(),
        opacity.contiguous(),
        ambient_diffuse_intensity.contiguous(),
        specular_intensity.contiguous(),
        means.contiguous(),
        scales.contiguous(),
        glob_scale,
        quats.contiguous(),
...
class _TextureGaussians(Function):
    ...
    def backward(
        ...
        return (
            ...
            v_colors,  # colors
            v_opacity,  # opacity
            None, # ambient_diffuse_intensity
            None, # specular_intensity
            v_means,    # means
            v_scales,   # scales
            None,  # glob_scale
            v_quats,    # quats
